...
import com.evitalz.homevitalz.cardfit.ui.activities.home.Patientnameid


@Dao
interface DeviceReadingdao{

    @Query("select * from Device_Readings where date(Date_time/1000 , 'unixepoch') >= date(:ms / 1000 , 'unixepoch') and date(Date_time/1000 , 'unixepoch') <= date(:endMs / 1000 , 'unixepoch') and P_id=:pregid ORDER BY Date_time ASC")
    fun getDeviceReadings( ms : Long , endMs : Long,pregid :Int) : LiveData<List<Device_Readings>>

    @Query("select * from Device_Readings where date(Date_time/1000 , 'unixepoch') >= date(:ms / 1000 , 'unixepoch') and date(Date_time/1000 , 'unixepoch') <= date(:endMs / 1000 , 'unixepoch') and P_id=:pregid ORDER BY Date_time DESC")
