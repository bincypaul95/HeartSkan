...
class LiquorViewmodel(application: Application, val dateinlong: Long, val deviceReadings: Device_Readings?, val update: Boolean):AndroidViewModel(application) {
    ...
    val drinkname: ObservableField<String> = ObservableField()
    ...
    init {
        ...
        btnsave.set("SAVE")
        if(update){
            drinkname.set(deviceReadings!!.dread2)
