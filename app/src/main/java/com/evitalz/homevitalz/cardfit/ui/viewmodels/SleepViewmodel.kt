...
class SleepViewmodel (application: Application, val dateinlong:Long, val deviceReadings: Device_Readings?, val update: Boolean):AndroidViewModel(application){
    val strDate : ObservableField<String> = ObservableField()
    val strtime : ObservableField<String> = ObservableField()
    val strStartDateTime : ObservableField<String> = ObservableField()
    val strEndDateTime : ObservableField<String> = ObservableField()
    ...
    val strDateTimeFormatter : SimpleDateFormat
    ...
    init {
        ...
        strDateTimeFormatter= SimpleDateFormat("MMM dd,hh:mm a ",Locale.getDefault())
        strDate.set(strFormatter.format(datetime.time))
        strtime.set(strTimeFormatter.format(datetime.time))
        strStartDateTime.set(strDateTimeFormatter.format(datetime.time))
        strEndDateTime.set(strDateTimeFormatter.format(datetime.time))
        ...
        btnsave.set("SAVE")
        if(update){
            strStartDateTime.set(deviceReadings!!.dread1)
            strEndDateTime.set(deviceReadings!!.dread2)
    ...
    fun setStartDateTime(dateTime: Date){
        datetime.time = dateTime
        strStartDateTime.set(strDateTimeFormatter.format(datetime.time))
    }
    fun setEndDateTime(dateTime: Date){
        datetime.time = dateTime
        strEndDateTime.set(strDateTimeFormatter.format(datetime.time))
    }
